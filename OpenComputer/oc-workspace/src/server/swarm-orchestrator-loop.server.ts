...
import { getSwarmProfilePath, listSwarmWorkerIds } from './swarm-foundation'
import {
...
} from './swarm-notifications'
import { applySwarmModeToLoopFlags, readSwarmMode } from './swarm-mode'
...
export async function handlePost({ request }: { request: Request }) {
  ...
  let orchestrationPrompt: { published: boolean; sessionKey: string } | null =
    null
