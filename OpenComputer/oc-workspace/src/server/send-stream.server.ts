...
} from '../routes/api/-send-stream-live-tools'
import { resolveSessionKey } from './session-utils'
import { isAuthenticated } from './auth-middleware'
import { requireJsonContentType } from './rate-limit'
import { publishChatEvent } from './chat-event-bus'
import { loadWorkspaceCatalog } from './workspace.server'
import {
          ...
          try {
            ...
            for await (const chunk of stream) {
              if (chunk.type === 'reasoning') {
                ...
                toolEventCount += 1
                const toolCallId =
                  chunk.toolCallId || `${runId}:${chunk.name}:${toolEventCount}`
            ...
            },
            {
              ...
              async onEvent({ event, data }) {
                ...
                if (event === 'run.completed') {
                  ...
                  try {
                    const sid =
                      readString(data.session_id) || sessionKeyFromEvent || ''
                    if (sid) {
                      let persistedMessages: Array<Record<string, unknown>> = []
                      try {
                        persistedMessages = (await getSessionMessagesFromAgent(
                          sid,
                        )) as unknown as Array<Record<string, unknown>>
                      } catch {
                        persistedMessages = []
                      ...
                      if (lastAssistantIndex >= 0) {
                        ...
                        for (const synthetic of syntheticEvents) {
                          ...
                          sendEvent('tool', synthetic)
                          skipPublish || publishChatEvent('tool', synthetic)
                        }
                      }
                    }
                  } catch (err) {
                    // Backfill is best-effort; don't fail the run.
                    console.warn('[send-stream] tool backfill failed:', err)
                  }
