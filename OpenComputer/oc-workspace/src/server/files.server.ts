...
import { isAuthenticated, requireLocalOrAuth } from './auth-middleware'
import {
  getClientIp,
  rateLimit,
  rateLimitResponse,
...
export async function handleGet({ request }: { request: Request }) {
  ...
  try {
    ...
    if (action === 'list' && hasGlob(inputPath)) {
      const globListing = await readGlobDirectory(inputPath, workspaceRoot)
      return json({
...
export async function handlePost({ request }: { request: Request }) {
  ...
  try {
    ...
    if (action === 'rename') {
      ...
      const toPath = ensureWorkspacePath(String(body.to || ''), workspaceRoot)
    ...
    const filePath = ensureWorkspacePath(String(body.path || ''), workspaceRoot)
