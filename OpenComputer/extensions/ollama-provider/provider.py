...
class OllamaProvider(BaseProvider):
    ...
    async def stream_complete(
        ...
        *,
        ...
        # Accumulate by index, then materialise as ToolCall objects
        # in the final `done` event. Same pattern as openai-provider.
            ...
            async with httpx.AsyncClient(timeout=httpx.Timeout(120.0)) as client:
                async with client.stream(
                    ...
                    try:
                        ...
                        logger.warning(
                            "ollama stream HTTP %s: %s", e.response.status_code, e
                        )
                        raise
    ...
    def _parse_response(self, data: dict) -> ProviderResponse:
        ...
        if not choices:
            raise RuntimeError(f"ollama returned empty choices: {data!r}")
        choice = choices[0]
