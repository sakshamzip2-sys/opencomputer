...
COMPUTER_USE_GUIDANCE = (
    ...
    "(default). You get a screenshot plus an indexed list of every "
    "interactable element — each entry carries a 1-based index, its AX "
    "role, and its label. The numbers are NOT drawn onto the screenshot; "
    "match an element to what you see in the image by its role and label.\n"
    ...
    "   Raw `coordinate=[x,y]` is in WINDOW-LOCAL SCREENSHOT pixels — the "
    "space of the capture image (top-left origin), not logical screen "
    "points. On Retina displays screenshot pixels are 2x the logical "
    "points; always read coordinates off the capture image.\n"
