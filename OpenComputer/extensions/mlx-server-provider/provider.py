"""MLX-server provider — OpenAI-compatible HTTP API.

Targets http://localhost:8081/v1 by default (override via MLX_SERVER_BASE_URL).
Default port 8081 to avoid the conflict with llama.cpp's 8080.
Auth is optional — if MLX_SERVER_API_KEY is set the value is sent as a
Bearer token, otherwise the Authorization header is omitted.

Backend: ``mlx_lm.server`` — Apple-Silicon-native inference via the MLX
framework. Models are typically Hugging Face IDs in the
...
DEFAULT_MODELS: tuple[str, ...] = ("mlx-community/Llama-3.1-8B-Instruct-4bit",)
DEFAULT_TIMEOUT_S = 120.0


class MLXServerProvider(BaseProvider):
    """OpenAI-compatible client targeting a locally-running mlx_lm.server."""
    ...
    def __init__(self, base_url: str | None = None, **_: Any) -> None:
        self.base_url = (
            base_url or os.environ.get("MLX_SERVER_BASE_URL") or MLX_SERVER_BASE_URL
        )

    def _api_key(self) -> str:
        # Local default — no auth required. Empty string signals "omit header".
        return os.environ.get("MLX_SERVER_API_KEY", "")
