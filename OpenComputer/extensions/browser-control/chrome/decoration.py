...
def decorate_openclaw_profile(
    ...
    *,
    ...
    try:
        ...
        # 5 = "Open the New Tab page" — prevents Chrome from restoring previous
        # multi-window sessions on startup. Without this, Chrome may restore old
        # windows from prior runs, creating ghost windows.
        _set_deep(default_prefs, ["session", "restore_on_startup"], 5)
...
def ensure_profile_clean_exit(user_data_dir: str) -> None:
    ...
    Also pins restore_on_startup=5 (NTP) so Chrome never restores previous
    multi-window sessions — the main driver of ghost windows on re-launch.
    ...
    try:
        ...
        _set_deep(prefs, ["session", "restore_on_startup"], 5)
