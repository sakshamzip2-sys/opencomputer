...
// Original: extension/src/background.ts
// Modifications:
//   - Brand renames ([opencli] → [opencomputer], OPENCLI_* → OPENCOMPUTER_*,
//     storage keys, error-message hints).
//   - Header rewritten for OpenComputer ownership.

/**
 * OpenComputer Browser Control — Service Worker (background script).
 *
 * Connects to the opencomputer daemon via WebSocket, receives commands,
 * dispatches them to Chrome APIs (debugger/tabs/cookies), returns results.
 *
 * Lease lifecycle (verbatim from OpenCLI):
 *   - bound:*      → pinned to user's tab, no idle timeout
 *   - browser:*    → owned, 10min idle (interactive)
 *   - operate:*    → owned, 10min idle (interactive)
 *   - (default)    → owned, 30s idle (adapter runs auto-cleanup)
 *
 * MV3 service-worker survival via chrome.storage.local + chrome.alarms.
 */

declare const __OPENCOMPUTER_COMPAT_RANGE__: string;
...
const CONTEXT_ID_KEY = 'opencomputer_context_id_v1';
...
async function connect(): Promise<void> {
  ...
  ws.onopen = () => {
    console.log('[opencomputer] Connected to daemon');
    ...
    ws?.send(JSON.stringify({
      ...
      compatRange: __OPENCOMPUTER_COMPAT_RANGE__,
    }));
  };
  ...
  ws.onmessage = async (event) => {
    try {
      ...
      console.error('[opencomputer] Message handling error:', err);
    }
  };

  ws.onclose = () => {
    console.log('[opencomputer] Disconnected from daemon');
    ws = null;
    scheduleReconnect();
  };
...
const REGISTRY_KEY = 'opencomputer_target_lease_registry_v1';
const LEASE_IDLE_ALARM_PREFIX = 'opencomputer:lease-idle:';
...
let windowFocused = false; // set per-command from daemon's OPENCOMPUTER_WINDOW_FOCUSED
...
async function ensureOwnedContainerWindowUnlocked(initialUrl?: string): Promise<{ windowId: number; initialTabId?: number }> {
  ...
  console.log(`[opencomputer] Created owned automation container window ${ownedContainerWindowId} (start=${startUrl})`);
...
async function getAutomationWindow(workspace: string, initialUrl?: string): Promise<number> {
  if (workspace.startsWith('bound:') && !automationSessions.has(workspace)) {
    throw new CommandFailure(
      ...
      `Bound workspace "${workspace}" is not attached to a tab. Run "opencomputer browser bind --workspace ${workspace}" first.`,
...
chrome.windows.onRemoved.addListener(async (windowId) => {
  ...
  for (const [workspace, session] of automationSessions.entries()) {
    if (session.windowId === windowId) {
      console.log(`[opencomputer] Automation container closed (${workspace})`);
      if (session.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(workspace);
      workspaceTimeoutOverrides.delete(workspace);
      scheduleIdleAlarm(workspace, IDLE_TIMEOUT_NONE);
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
  ...
  for (const [workspace, session] of automationSessions.entries()) {
    if (session.preferredTabId === tabId) {
      ...
      workspaceTimeoutOverrides.delete(workspace);
      scheduleIdleAlarm(workspace, IDLE_TIMEOUT_NONE);
      console.log(`[opencomputer] Workspace ${workspace} lease detached from tab ${tabId} (tab closed)`);
    }
  }
  if (ownedContainerWindowId !== null) {
...
function initialize(): void {
  ...
  console.log('[opencomputer] OpenComputer extension initialized');
}
...
async function resolveTab(tabId: number | undefined, workspace: string, initialUrl?: string): Promise<ResolvedTab> {
    ...
    try {
      ...
      if (session && !session.owned) {
        throw new CommandFailure(
          ...
          'Run "opencomputer browser bind" again on a debuggable http(s) tab.',
        );
      }
      if (session && !matchesSession && session.preferredTabId === null && isDebuggableUrl(tab.url)) {
        ...
        console.warn(`[opencomputer] Tab ${tabId} drifted to window ${tab.windowId}, moving back to ${session.windowId}`);
        try {
          ...
          console.warn(`[opencomputer] Failed to move tab back: ${moveErr}`);
        }
      } else if (!isDebuggableUrl(tab.url)) {
        console.warn(`[opencomputer] Tab ${tabId} URL is not debuggable (${tab.url}), re-resolving`);
      }
    } catch (err) {
      ...
      if (existingSession && !existingSession.owned) {
        ...
        throw new CommandFailure(
          ...
          'Run "opencomputer browser bind" again, then retry the command.',
        );
      }
      console.warn(`[opencomputer] Tab ${tabId} no longer exists, re-resolving`);
    }
  }
    ...
    try {
      ...
      if (!session.owned) {
        throw new CommandFailure(
          ...
          'Switch the tab to an http(s) page or run "opencomputer browser bind" on another tab.',
        );
      }
    } catch (err) {
      if (err instanceof CommandFailure) throw err;
  ...
  if (reuseTab?.id) {
    ...
    try {
      ...
      console.warn(`[opencomputer] data: URI was intercepted (${updated.url}), creating fresh tab`);
    } catch {
...
async function handleNavigate(cmd: Command, workspace: string): Promise<Result> {
  ...
  await new Promise<void>((resolve) => {
    ...
    timeoutTimer = setTimeout(() => {
      timedOut = true;
      console.warn(`[opencomputer] Navigate to ${targetUrl} timed out after 15s`);
      finish();
    }, 15000);
  });
  ...
  if (postNavigationSession?.owned === false && tab.windowId !== postNavigationSession.windowId) {
    return {
      ...
      errorHint: 'Run "opencomputer browser bind" again on the intended tab.',
    };
  }
  if (postNavigationSession && tab.windowId !== postNavigationSession.windowId) {
    console.warn(`[opencomputer] Tab ${tabId} drifted to window ${tab.windowId} during navigation, moving back to ${postNavigationSession.windowId}`);
    try {
      await chrome.tabs.move(tabId, { windowId: postNavigationSession.windowId, index: -1 });
      tab = await chrome.tabs.get(tabId);
    } catch (moveErr) {
      console.warn(`[opencomputer] Failed to recover drifted tab: ${moveErr}`);
    }
  }
...
async function releaseWorkspaceLease(workspace: string, reason: string = 'released'): Promise<void> {
  ...
  if (session.owned) {
    ...
    if (tabId !== null) {
      ...
      console.log(`[opencomputer] Released owned tab lease ${tabId} (${workspace}, ${reason})`);
    } else {
      ...
      console.log(`[opencomputer] Released legacy owned window lease ${session.windowId} (${workspace}, ${reason})`);
    }
  } else if (session.preferredTabId !== null) {
    await safeDetach(session.preferredTabId);
    console.log(`[opencomputer] Detached borrowed tab lease ${session.preferredTabId} (${workspace}, ${reason})`);
  }
...
async function handleBind(cmd: Command, workspace: string): Promise<Result> {
  ...
  resetWindowIdleTimer(workspace);
  console.log(`[opencomputer] Workspace ${workspace} explicitly bound to tab ${boundTab.id} (${boundTab.url})`);
