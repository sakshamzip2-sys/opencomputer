...
Targets http://localhost:1234/v1 by default (override via LMSTUDIO_BASE_URL).
LM Studio's OpenAI-compatible server defaults to ``api_key="lm-studio"``
when started in compatibility mode, so we use that as the fallback when
no env var is set. The user can clear LMSTUDIO_API_KEY explicitly to ""
to force omission of the Authorization header for an unauthenticated
local setup.
"""
...
LMSTUDIO_DEFAULT_API_KEY = "lm-studio"  # LM Studio's literal default
DEFAULT_MODELS: tuple[str, ...] = ("local-model",)
DEFAULT_TIMEOUT_S = 120.0


class LMStudioProvider(BaseProvider):
    """OpenAI-compatible client targeting a locally-running LM Studio server."""
    ...
    def __init__(self, base_url: str | None = None, **_: Any) -> None:
        self.base_url = (
            base_url or os.environ.get("LMSTUDIO_BASE_URL") or LMSTUDIO_BASE_URL
        )

    def _api_key(self) -> str:
        # LM Studio defaults to the literal string "lm-studio" — use that
        # as the fallback. A user with a custom-key setup overrides via env.
        return os.environ.get("LMSTUDIO_API_KEY", LMSTUDIO_DEFAULT_API_KEY)
