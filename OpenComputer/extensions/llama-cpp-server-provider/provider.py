"""llama.cpp local-server provider — OpenAI-compatible HTTP API.

Targets http://localhost:8080/v1 by default (override via
LLAMA_CPP_SERVER_BASE_URL). Auth is optional — if
LLAMA_CPP_SERVER_API_KEY is set the value is sent as a Bearer token,
otherwise the Authorization header is omitted entirely (llama-server's
default is no auth).
"""
...
class LlamaCppServerProvider(BaseProvider):
    """OpenAI-compatible client targeting a locally-running llama-server."""
    ...
    def __init__(self, base_url: str | None = None, **_: Any) -> None:
        self.base_url = (
            base_url
            or os.environ.get("LLAMA_CPP_SERVER_BASE_URL")
            or LLAMA_CPP_SERVER_BASE_URL
        )

    def _api_key(self) -> str:
        # Local default — no auth required. Empty string signals "omit header".
        return os.environ.get("LLAMA_CPP_SERVER_API_KEY", "")
