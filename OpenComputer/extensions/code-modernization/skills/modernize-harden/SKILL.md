...
description: Step 7 of the modernization pipeline — security hardening pass. Adversarially scan the legacy system for OWASP Top 10, CWE matches, dependency CVEs, secrets in source, injection sinks. Rank by severity and produce a reviewable remediation patch for the critical ones. Use when the user says security audit this legacy code, find vulnerabilities, or harden this old codebase. Args is the system dir name. Never edits legacy/ directly — writes findings plus proposed patch to analysis/NAME.
version: 0.1.0
---
