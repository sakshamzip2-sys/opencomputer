...
description: Step 5 of modernization — multi-agent greenfield rebuild. Treat the legacy system as the spec source, not the structural template. Extract requirements, design from scratch, scaffold the new architecture, validate via human-in-the-loop checkpoints. Use when the user says rewrite this from scratch, reimagine this system, or build a greenfield replacement. Args are system dir and target-vision paragraph. NOT for ports — use modernize-transform for module-by-module porting.
version: 0.1.0
---
