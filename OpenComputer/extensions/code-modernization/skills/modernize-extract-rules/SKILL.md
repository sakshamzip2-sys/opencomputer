...
description: Step 3 of the modernization pipeline — mine the business rules embedded in a legacy system into a structured, testable Given/When/Then specification. The institutional knowledge currently locked in code. Use when the user says extract business rules, what are the rules in this code, or pull the domain logic out. Args are the system dir name (required) and an optional module pattern to focus the scope. Reuses the business-rules-extractor subagent for the deep extraction work.
version: 0.1.0
---
