...
import requests
from compat import call_llm, get_hermes_home

try:
    from compat import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 — fail-open if policy module unavailable

try:
    from compat import is_safe_url as _is_safe_url
except Exception:
...
from browser_providers.firecrawl import FirecrawlProvider
from compat import normalize_browser_cloud_provider
...
def _get_command_timeout() -> int:
    ...
    try:
        from compat import read_raw_config
        cfg = read_raw_config()
...
# Hermes-vendored split: provider-resolution helpers above must
# define their constants before this import lands; hence noqa.
from compat import is_termux as _is_termux_environment  # noqa: E402
...
def _run_browser_command(
    ...
    from compat import is_interrupted
    if is_interrupted():
...
def browser_vision(question: str, annotate: bool = False, task_id: str | None = None) -> str:
    ...
    from compat import get_hermes_dir
...
def check_browser_requirements() -> bool:
    ...
    *and* the provider's required credentials must be present.
    ...
    provider = _get_cloud_provider()
    return not (provider is not None and not provider.is_configured())
...
# Registry
# ---------------------------------------------------------------------------
from compat import (  # noqa: E402 — registry import deferred until BROWSER_TOOL_SCHEMAS is defined
    registry,
    tool_error,
)
