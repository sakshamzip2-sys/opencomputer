...
#: have non-zero rates; ``docker`` / ``linux_bwrap`` / ``macos_sandbox_exec``
#: / ``ssh`` / ``none`` run on hardware the operator already owns and are
#: free — intentionally absent so :meth:`SandboxCostGuard.rate_for` returns
#: ``0.0`` for them. F16 (M2 audit): a paid backend with no entry silently
#: bypasses the session cap, so every paid backend MUST be listed here.
