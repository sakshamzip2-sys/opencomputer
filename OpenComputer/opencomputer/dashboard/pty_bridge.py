"""PTY bridge for the dashboard chat tab (Wave 6.D — Hermes port).

Wraps a child process behind a pseudo-terminal so its ANSI output can be
streamed to a browser-side terminal emulator (xterm.js) and typed
keystrokes can be fed back in. The only caller today is the
``/api/pty`` WebSocket endpoint in :mod:`opencomputer.dashboard.server`.

Verbatim port of ``sources/hermes-agent/hermes_cli/pty_bridge.py``
(commit f49afd312). Only docstrings and module names changed; behavioural
semantics — signal escalation, timeouts, errno handling, byte-safe I/O —
are preserved exactly.

Design constraints:

* **POSIX-only.** Native Windows Python has no ``openpty(3)``;
  :class:`PtyUnavailableError` is raised with a user-readable
  install/platform message so the dashboard can render a banner instead
  of crashing.
* **Zero Node dependency on the server side.** Uses :mod:`ptyprocess`,
  a pure-Python wrapper around OS calls. The browser talks to whatever
  binary the operator configured (typically ``oc chat``), so every TUI
...
* **Byte-safe I/O.** Reads and writes go through the PTY master fd
  ...
  streaming ANSI is inherently byte-oriented and UTF-8 boundaries may
  land mid-read.
"""
...
class PtyBridge:
    ...
    :mod:`ptyprocess` methods concurrently, we only call ``os.read``
    and ``os.write`` on the master fd, which is safe.
    """
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError(
                    ...
                    "OpenComputer dashboard chat is supported on Linux/macOS "
                    "or Windows via WSL."
                )
    ...
    def close(self) -> None:
        """Terminate the child (SIGHUP→SIGTERM→SIGKILL) and close fds.
