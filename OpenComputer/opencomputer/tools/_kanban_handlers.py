...
running under the dispatcher (env var ``OC_KANBAN_TASK`` set). A
...
def tool_error(msg: str) -> str:
    """Return a JSON error string in the standard tool-call shape."""
    return json.dumps({"error": msg})


def _check_kanban_mode() -> bool:
    """Tools are available iff the current process has ``OC_KANBAN_TASK``
...
def _default_task_id(arg: str | None) -> str | None:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("OC_KANBAN_TASK")
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set OC_KANBAN_TASK in the env)"
        )
    try:
        kb, conn = _connect()
        try:
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = args.get("author") or os.environ.get("OC_PROFILE") or "worker"
    try:
        kb, conn = _connect()
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("OC_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=os.environ.get("OC_PROFILE") or "worker",
            )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to OC_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to OC_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
