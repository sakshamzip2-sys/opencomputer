...
def _register_builtin_tools() -> None:
    ...
    if "Read" in registry.names():
        return
    # Apply tools.deny (exact match) and agent.disabled_toolsets (prefix
    # match) from agent config. Idempotent — re-running with the same list
    # is fine.
    try:
        ...
        registry.set_deny_prefixes(
            tuple(getattr(_cfg.loop, "disabled_toolsets", ()) or ())
        )
...
def webui(
    host: str = typer.Option(
        None,
        "--host",
        help="Bind address (default: $OC_WEBUI_HOST or 127.0.0.1).",
    ),
    port: int = typer.Option(
        None,
        "--port",
        help="HTTP port (default: $OC_WEBUI_PORT or 8787).",
    ),
    ...
    foreground: bool = typer.Option(
        ...
        "--foreground",
        help=(
            "Run in this process (for systemd/launchd/supervisord). "
            "Auto-detected when launched under those supervisors."
        ),
    ),
    webui_dir: str = typer.Option(
        None,
        "--webui-dir",
        help=(
            "Path to oc-webui repo. Defaults to a sibling 'oc-webui' "
            "next to OpenComputer/, or $OC_WEBUI_DIR."
        ),
    ),
) -> None:
    """Launch the OpenComputer web UI in your browser.

    Forwards to ``oc-webui/bootstrap.py`` with the active OC profile's
    home, Python interpreter, and OPENCOMPUTER_HOME pre-exported so the
    WebUI inherits the same plugin/skill/config/session surface as
    ``oc chat``.

    Discovery order for the oc-webui directory:

    1. ``--webui-dir`` argument
    2. ``$OC_WEBUI_DIR`` env var
    3. Sibling ``oc-webui/`` next to the OpenComputer source tree
    4. ``$OPENCOMPUTER_HOME/plugins/oc-webui``
    5. ``~/.opencomputer/oc-webui``

    Fails loudly with the searched paths if none match.
    """
    import os
    import subprocess
    import sys
    ...
    profile_home = _profile_home_fn()
    pkg_dir = Path(__file__).resolve().parent.parent  # OpenComputer/

    def _is_webui(p: Path) -> bool:
        return p.is_dir() and (p / "bootstrap.py").is_file()

    # Explicit args are STRICT: if the user provided --webui-dir or
    # $OC_WEBUI_DIR pointing somewhere invalid, fail loudly instead of
    # silently falling through to a different directory. Silent fallback
    # would invoke the wrong webui under the user's nose.
    resolved: Path | None = None
    if webui_dir:
        explicit = Path(webui_dir).expanduser().resolve()
        if not _is_webui(explicit):
            console.print(
                f"[red]error:[/red] --webui-dir [cyan]{explicit}[/cyan] "
                f"is not a valid oc-webui repo (no bootstrap.py)."
            )
            raise typer.Exit(code=1)
        resolved = explicit
    elif (env_dir := os.environ.get("OC_WEBUI_DIR", "").strip()):
        explicit = Path(env_dir).expanduser().resolve()
        if not _is_webui(explicit):
            console.print(
                f"[red]error:[/red] $OC_WEBUI_DIR=[cyan]{explicit}[/cyan] "
                f"is not a valid oc-webui repo (no bootstrap.py)."
            )
            raise typer.Exit(code=1)
        resolved = explicit
        ...
        candidates: list[Path] = [
            pkg_dir / "oc-webui",
            profile_home / "plugins" / "oc-webui",
            Path.home() / ".opencomputer" / "oc-webui",
        ]
        resolved = next(
            (c.resolve() for c in candidates if _is_webui(c)),
            None,
        )
        if resolved is None:
            searched = "\n  ".join(str(p) for p in candidates)
            console.print(
                "[red]error:[/red] oc-webui not found. Searched:\n  " + searched
            )
            console.print(
                "\nFix: either set [bold]--webui-dir[/bold] / "
                "[bold]$OC_WEBUI_DIR[/bold], or clone hermes-webui into "
                f"[cyan]{pkg_dir / 'oc-webui'}[/cyan]."
            )
            raise typer.Exit(code=1)

    # Wire the active profile into the WebUI's environment so the agent
    # AND the WebUI's session DB / skills / cron all read the same home.
    env = os.environ.copy()
    env["OPENCOMPUTER_HOME"] = str(profile_home)
    env.setdefault("HERMES_HOME", str(profile_home))  # legacy alias for upstream
    if host:
        env["OC_WEBUI_HOST"] = host
        env["HERMES_WEBUI_HOST"] = host
    if port is not None:
        env["OC_WEBUI_PORT"] = str(port)
        env["HERMES_WEBUI_PORT"] = str(port)
    # Force the WebUI's bootstrap to use this Python — guarantees the
    # same venv (so the OC + webui imports both resolve).
    env["HERMES_WEBUI_PYTHON"] = sys.executable

    cmd = [sys.executable, str(resolved / "bootstrap.py")]
    if no_browser:
    ...
    console.print(
        f"[bold cyan]oc webui[/bold cyan] → "
        f"[dim]{resolved}/bootstrap.py[/dim]"
    )
    console.print(f"[dim]profile home: {profile_home}[/dim]")
    try:
        result = subprocess.run(cmd, env=env, cwd=str(resolved), check=False)
    except KeyboardInterrupt:
        console.print("\n[dim]webui stopped[/dim]")
