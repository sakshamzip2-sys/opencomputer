...
description: Plan, set up, and monitor a multi-agent video production pipeline backed by OC Kanban. Use when the user wants to produce ANY video (film/marketing/music/explainer/ASCII/3D/etc.) and the work warrants decomposition into specialist profiles coordinated through a kanban board. Generates setup scripts, creates profiles, fires the initial kanban task, and helps monitor execution.
version: 1.0.0
...
short to a music video to an ASCII loop — in a OC Kanban pipeline that
...
3. **Generates** a setup script that creates OC profiles, project workspace, and the initial kanban task
...
For mapping role → which OC skills + toolsets it loads, see
...
3. Creates each OC profile via `oc profile create --clone`
...
7. Fires the initial `oc kanban create` task assigned to the director
...
```bash
oc kanban watch --tenant <project-tenant>     # live events
oc kanban list  --tenant <project-tenant>     # board snapshot
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `oc kanban create` call to a specialist profile. The
