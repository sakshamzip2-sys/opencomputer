...
description: Build interactive single-file HTML playgrounds — explorable visual tools where the user adjusts controls and copies out a prompt. Use when the user asks for "a playground", "an interactive explorer", "a design playground", "a data explorer", "a concept map", "a diff review tool", "a document critique UI", or "a code-map page". Six bundled templates cover design / data / learning / document review / diff review / codebase. Pairs well with OC's `html-artifact-design` skill for cosmetic theming.
version: 0.1.0
---

# Playground Builder

A playground is a self-contained HTML file with interactive controls on one side, a live preview on the other, and a prompt output at the bottom with a copy button. The user adjusts controls, explores visually, then copies the generated prompt back into the agent (CLI, Telegram, web, etc.).
