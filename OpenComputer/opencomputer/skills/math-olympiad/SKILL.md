---
name: math-olympiad
description: Solve competition math problems (IMO, Putnam, USAMO, AIME) with adversarial verification. Use when asked to solve an olympiad problem, prove an inequality, verify a competition proof, find a counterexample, or check if a proof is correct. Pure reasoning — no computation tools — then a fresh-context verifier attacks the proof using specific failure patterns. Outputs calibrated confidence and will abstain rather than bluff. Optional LaTeX PDF output if pdflatex is available.
version: 0.1.0
---

# Math Olympiad Solver

> **OC adaptation note (read first):** Where this skill says "launch subagents
> in parallel via the Agent tool", on OpenComputer the equivalent is the
> built-in `DelegateTool` (delegates to a sub-agent with its own context). The
> *prompt restriction trick* — "NO COMPUTATION" written verbatim into the
> prompt — works identically across both harnesses because subagents in either
> system can in principle access tools, and the only enforcement is the prompt
> itself. The pattern of stripping the solver's reasoning trace before
> verification is the load-bearing trick — it generalises beyond math; see
> `references/verifier_patterns.md` for the catalogue.
