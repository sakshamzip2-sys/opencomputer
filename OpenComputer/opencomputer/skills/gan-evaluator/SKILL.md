---
name: gan-evaluator
description: GAN Harness — Evaluator role. Tests the live running application via Playwright, scores against rubric, and provides actionable feedback to the Generator. Use as the critic in iterative build-test-fix loops.
---
...
# Use Playwright to interact with the live app
...
Use Playwright (via the Playwright MCP if available, or via direct CLI):

```bash
# Navigate
npx playwright test --headed --browser=chromium

# Or via MCP tools if available (browser_navigate, browser_click, browser_fill, browser_take_screenshot)
```
...
### `screenshot` mode
Take screenshots only, analyze visually. Less thorough but works without a live driver.
