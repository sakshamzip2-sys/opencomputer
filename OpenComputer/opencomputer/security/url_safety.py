...
Limitations (documented, not fixable at pre-flight level):
  ...
  - Redirect-based bypass is mitigated by httpx event hooks that re-validate
    each redirect target in WebFetch. Web tools that use third-party SDKs
    (Firecrawl/Tavily) rely on those providers' redirect handling.

Ported from Hermes-agent's ``tools/url_safety.py`` (Tier-S Task 4). Adaptations:
  * ``HERMES_ALLOW_PRIVATE_URLS`` → ``OPENCOMPUTER_ALLOW_PRIVATE_URLS``
  * ``hermes_cli.config.read_raw_config`` → reads OC's
    ``~/.opencomputer/config.yaml`` directly via ``yaml.safe_load``. The
    ``security`` and ``browser`` sections are not declared in OC's
    dataclass schema (they live as freeform YAML keys).
  * ``_TRUSTED_PRIVATE_IP_HOSTS`` is empty; the framework remains for
    future use (e.g. a corporate proxy hostname that must resolve to
    private space).
"""
...
# Empty by default in OpenComputer; kept as a frozenset so future
# integrations can extend without re-introducing the original Hermes
# entries (which were specific to QQ media downloads).
...
def _read_raw_config() -> dict:
    """Best-effort read of ``~/.opencomputer/config.yaml`` as a raw dict.

    Returns an empty dict on any error (missing file, parse error, etc.).
    The freeform ``security`` / ``browser`` blocks aren't part of OC's
    dataclass-typed Config, so we read the YAML directly rather than
    going through ``load_config()`` (which would drop unknown keys).
    """
    try:
        ...
        if isinstance(raw, dict):
            return raw
    except Exception:  # noqa: BLE001 — best-effort lookup, never raise
        pass
    return {}
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``OPENCOMPUTER_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("OPENCOMPUTER_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``OPENCOMPUTER_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
