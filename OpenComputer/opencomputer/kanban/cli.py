"""CLI for the OC Kanban board — ``oc kanban …`` subcommand.

Exposes the full 15-verb surface documented in the design spec
(``docs/oc-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this OC_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across OC profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://opencomputer.dev/docs/user-guide/features/kanban "
            "or docs/oc-kanban-v1-spec.pdf for the full design."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $OC_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $OC_PROFILE or 'user')")
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.opencomputer/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: oc kanban <action> [options]\n"
    ...
    # OC_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("OC_PROFILE_NAME", "OC_PROFILE"):
        v = os.environ.get(env)
        if v:
            return v
    try:
        from opencomputer.kanban.profiles import get_active_profile_name
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.opencomputer/profiles/.")
