"""Smoke tests for the P1.b provider cohort (xAI, Z.AI, Kimi, MiniMax, DashScope).

Each provider plugin has the same shape — env-var precondition + base URL
default + name/default_model class attrs. We parameterize a single suite
across all five rather than copy/paste five identical files.

Same isolation pattern as test_deepseek_provider — load each plugin's
provider.py with a unique sys.modules name so the shared 'provider'
namespace from openai-provider doesn't get clobbered between loads.
"""
...
# (plugin_dir, class_name, expected provider id, env var, default base host)
COHORT = [
...
def test_does_not_fall_back_to_openai_api_key(
    ...
    """Setting OPENAI_API_KEY must NOT satisfy the provider-specific check."""
    monkeypatch.delenv(env_var, raising=False)
...
def test_all_five_providers_appear_in_wizard_discovery():
    """The wizard's _discover_providers must surface every cohort member."""
