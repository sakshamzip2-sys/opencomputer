"""Phase 12a — Recall tool + post-response reviewer + agent cache.

Three new modules tested + their AgentLoop integration:

1. opencomputer/tools/recall.py    — RecallTool (search / note / recall_session)
...
def _call(action: str, **kwargs) -> ToolCall:
    return ToolCall(id="tc-1", name="Recall", arguments={"action": action, **kwargs})
...
def test_memory_tool_schema_lists_three_actions(tmp_path: Path) -> None:
    ...
    tool = RecallTool(db=db, memory=mem)
    schema = tool.schema
...
async def test_memory_note_rejects_oversize(tmp_path: Path) -> None:
    from opencomputer.agent.memory import MemoryManager
    from opencomputer.agent.state import SessionDB
    from opencomputer.tools.recall import MAX_NOTE_CHARS, RecallTool
...
def test_register_builtin_tools_includes_memory() -> None:
    ...
    assert "Recall" in set(registry.names())
