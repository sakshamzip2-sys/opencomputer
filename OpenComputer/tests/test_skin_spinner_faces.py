...
def test_all_builtin_skins_ship_a_rich_face_pool():
    """Every built-in skin's spinner pools are wide enough to feel varied —
    OC historically shipped only 3-4 per cycle; Hermes ships ~10-15."""
    for name in list_builtin_names():
        spec = load_skin(name)
        assert len(spec.spinner_waiting_faces) >= 8, name
        assert len(spec.spinner_thinking_faces) >= 8, name
        assert len(spec.spinner_thinking_verbs) >= 8, name
        # variety, not padding — no duplicate faces within a cycle
        assert len(set(spec.spinner_waiting_faces)) == len(
            spec.spinner_waiting_faces
        ), f"{name}: duplicate waiting face"
        assert len(set(spec.spinner_thinking_faces)) == len(
            spec.spinner_thinking_faces
        ), f"{name}: duplicate thinking face"
