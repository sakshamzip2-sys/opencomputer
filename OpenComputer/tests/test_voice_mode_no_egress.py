"""tests/test_voice_mode_no_egress.py — AST no-network-egress contract guard.

The voice-mode plugin's source MUST NOT directly import HTTP/network
clients. Whisper API calls go through ``opencomputer.voice.stt`` (which
uses the cost-guarded OpenAI client) — the plugin source itself stays
network-free.

(Provider plugins / the existing voice helper module are exempt — that's
...
3. Update ``extensions/voice-mode/README.md`` privacy contract.
4. Update CHANGELOG.
...
# Importable names of HTTP/network libraries the voice-mode plugin must
# not use directly. The existing ``opencomputer.voice.stt`` helper is the
# sanctioned channel for OpenAI Whisper calls.
...
def _plugin_root() -> Path:
    """Return the path to extensions/voice-mode/ from this test file."""
    return Path(__file__).resolve().parent.parent / "extensions" / "voice-mode"
...
def test_no_network_imports_in_voice_mode_plugin():
    """Sweep extensions/voice-mode/*.py for HTTP-client imports."""
    violations: list[str] = []
    for path in _plugin_root().rglob("*.py"):
        for line_no, stmt in _scan(path):
            violations.append(f"{path}:{line_no}: {stmt}")
    assert not violations, (
        "voice-mode must NOT import network libs:\n  "
        + "\n  ".join(violations)
        + "\n\nIf the network import is intentional: update the deny-list, "
        + "update extensions/voice-mode/README.md privacy contract, "
