...
- `[2026-04-25 18:00] feat/hermes-evolution-b3` touched `opencomputer/evolution/{trajectory,cli}.py` (extended — appended `register_with_bus` + `_on_tool_call_event` + `_on_session_end` + `is/set_collection_enabled` + `bootstrap_if_enabled` to trajectory.py; added `trajectories show / enable / disable` CLI commands), `tests/test_evolution_b3_{subscriber,cli}.py` (2 new test files / 16 new tests). Full suite at 1860 passing. Worktree at `/tmp/oc-b3`. **Subscribes to F2 bus** (`opencomputer.ingestion.bus.default_bus`, landed in 3.A); never modifies the publisher. Exception-isolated. Privacy-preserving event conversion (200-char filter applied). `bootstrap_if_enabled()` is the startup hook — Session A may wire it into AgentLoop init in a future PR (their reserved territory). **Session B plan now COMPLETE** (B1 + B2 + B3 + B4 all merged once this lands).
...
- `[2026-04-25 14:30] feat/f6-c4-opencli-use-cases` touched `extensions/opencli-scraper/use_cases/*` (new — research_automation, content_monitoring, context_enrichment, competitor_research, market_signals — 5 function libraries composing C2 tools into higher-level patterns), `tests/test_opencli_use_cases_*.py` (5 new test files / 76 tests). Full suite at 1775 passing. Worktree at `/tmp/oc-c4`. **NOT registered as tools** — these are helper APIs callable from tests / Session A's Phase 4 wiring / user code. **Zero Session-A or Session-B reserved files touched.**
