...
| W3 | `client/` + `_tool.py` + `plugin.py` + e2e | [BRIEF-06](BRIEF-06-client-and-utils.md) | 3 | all | s5 | feat/browser-port-wave3 | merged | [#397](https://github.com/sakshamzip2-sys/opencomputer/pull/397) | 81 unit + 1 e2e + plugin smoke. Filename `_tool.py` (singular underscore) chosen over `tools.py` to dodge sys.modules collision |
| W3.x | hotfixes (dispatcher bootstrap, CDP openers, liveness probe, orphan tool_use) | (inline) | 3 | — | s1+subagents | (multiple) | merged | [#398](https://github.com/sakshamzip2-sys/opencomputer/pull/398) [#401](https://github.com/sakshamzip2-sys/opencomputer/pull/401) [#414](https://github.com/sakshamzip2-sys/opencomputer/pull/414) [#415](https://github.com/sakshamzip2-sys/opencomputer/pull/415) | functional v0.1 + reliability fixes |
| **W4** | **adapter promotion + plugin packaging + starter pack** | [wave4-adapters/BLUEPRINT](wave4-adapters/BLUEPRINT.md) | **4** | W3 merged | s-w4 | feat/browser-port-wave4-adapters | review | (pending) | adapter-runner plugin + 9 new Browser actions (incl. verify) + 8-adapter starter pack + adapter-author skill + adapter-pack scaffolding template + 5 typed errors. See [DEFERRED.md](wave4-adapters/DEFERRED.md) for what's intentionally NOT in this wave |
| **W6** | **control-extension port + daemon WS + Track 1 auto-load** | [wave6-control-extension/BLUEPRINT](wave6-control-extension/BLUEPRINT.md) | **6** | W4 merged | s1 | feat/wave6-control-extension | review | [#443](https://github.com/sakshamzip2-sys/opencomputer/pull/443) | OpenCLI extension verbatim port (Apache 2.0); Python daemon WS at `127.0.0.1:18792/ext`; new `control-extension` BrowserDriver; Track 1 `--load-extension` baked into managed Chrome args. **All 14 OpenCLI actions ship** (full general-agent capability — file uploads via `set-file-input`, robust typing via `insert-text`, current-tab UX via `bind`, iframe support via `frames`, plus `sessions`/`close-window` for diagnostics). 48 new tests + 587 existing pass; ruff clean. |
| W5+ | autofix / external CLI / OpenCLI shell-out / etc. | (deferred) | 5+ | W4 done | — | — | not started | — | See [wave4-adapters/DEFERRED.md](wave4-adapters/DEFERRED.md) for the v0.5+ roadmap |
| Wn | `providers/base.py` + Hermes seam | (long-deferred) | post-v0.1 | — | — | — | not started | — | Browserbase / Firecrawl / Camoufox alternate backends. Parked unless explicit demand |
