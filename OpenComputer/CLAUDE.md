...
**Mid-session profile swap (Ctrl+P / `/handoff` / classifier-driven auto-swap)** is real after 2026-05-15: env vars, .env, allowlisted Config fields, provider client, MCP fleet, SessionDB, ConsentGate audit chain, browser-profile, Honcho, and Langfuse all rebind on swap. The `ProfileRebindRegistry` at `opencomputer/agent/profile_rebind.py` is ordered + exception-isolated; built-ins install in every AgentLoop via `_install_builtin_rebind_handlers`. Plugins subscribe via `PluginAPI.register_profile_rebind_handler(name, handler, priority=)`. Plugin reload (new plugin set in target profile) still requires process restart — documented hard-deferral. See `docs/superpowers/specs/2026-05-15-profile-handoff-real-swap-design.md` and `docs/plans/profile-handoff-investigation.md` v3 closure log.
...
16. **Profile-rebind handlers run on EVERY AgentLoop, not just the chat session.** `_install_builtin_rebind_handlers` is called from `AgentLoop.__init__`, so gateway-spawned loops + every `DelegateTool` subagent gets the full set: `dotenv` (20) → `config` (50) → `provider` (60) → `mcp` (110) → `session_db` (120) → `consent_gate` (130). The MCP handler resolves its target via `manager_registry.current_active_manager()`, NOT a per-loop reference, so subagents diff-cycle the SHARED fleet on swap. Plugin handlers (browser-harness, memory-honcho, langfuse) are queued on `PluginAPI._pending_profile_rebind_handlers` at plugin load and drained into each loop's registry at `__init__`. Cl ose-time imports in plugin rebind handlers MUST be resolved at plugin-load (not lazy-imported) — `sys.path` no longer has the plugin dir at swap time; the browser-harness handler captures `_emergency_cleanup_all_sessions` into the closure rather than importing it lazily, for this reason.
